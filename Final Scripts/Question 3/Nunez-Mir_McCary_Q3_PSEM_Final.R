...
#check vif
vif1<-lm(AvgOfRootCNRatio ~ InvFraction + AvgOfMeanRootDiameter + AvgOfMycorrhizalColonization, data= data)
vif(vif1)

vif2<-lm(dbRDA1 ~  AvgOfRootCNRatio  + AvgOfMycorrhizalColonization + AvgOfRootDensity, data = data)
vif(vif2)

vif3<-lm(dbRDA2 ~ AvgOfRootCNRatio + AvgOfMycorrhizalColonization + AvgOfRootDensity, data = data)
vif(vif3)
