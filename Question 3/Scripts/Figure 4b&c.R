...
###coordinates plus ecosystem characteristics
#import
ecosystem<- file.path(".", "Data", "AP-MAT-MEM_plotdata.csv")
ecos<-read_csv(ecosystem)

#combine microbes and invasives
df1<-merge(ecos, microbes, by = "plotID")

##combine all data to match labels
mic_invas<- merge(df1, invasives,  by='plotID')
...
invas<-
  mic_invas%>%
  select(Root_C:RTD, MAT, MEM1, MEM2, AP)%>%
  mutate_at(vars(Root_C:RTD), log)

mod1 <- rda(decostand(spe, "hel") ~., invas)
mod1 <- rda(decostand(spe, "hel") ~.-MEM2, invas)##individually removed by the greatest VIF
mod1 <- rda(decostand(spe, "hel") ~.-MEM2-Root_C, invas)##individually removed by the greatest VIF
...
sites.long2 <- sites.long(plot1, env.data=inv1)

axis.long2 <- axis.long(mod1, choices=c(1, 2))

spec.envfit <- envfit(plot1, env=inv1)
...
##combine all data to match labels
mic_natives<- merge(df1, natives,  by='plotID')
...
mod2 <- rda(decostand(spe, "hel") ~. -RMF, nat)#remove first, highest VIF 
mod2 <- rda(decostand(spe, "hel") ~. -Root_C -RMF, nat)#remove second highest VIF 
...
nat1<-
  nat%>%
  select(MEM2, Root_N, Myco_I, MEM1, AP)
...
axis.long2 <- axis.long(mod2, choices=c(1, 2))
